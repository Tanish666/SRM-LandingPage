        ...
        <section className="w-full flex flex-col lg:flex-row bg-[#FAFAFA] overflow-hidden h-fit">
            ...
            <div className="w-full lg:w-1/2 grid grid-cols-2 gap-0 content-start">
                {cards.map((card, idx) => (
                    <div
                        key={idx}
                        className={`relative h-96 flex flex-col justify-end p-4 lg:p-6 overflow-hidden ${card.bg}`}
                    >
                        {/* Placeholder for Card Logo (Top Left) */}
                        <div className="absolute top-4 left-4 lg:top-6 bg-background justify-center items-center flex lg:left-6 z-20 w-36 h-16 rounded-md">
                            <img loading="lazy" decoding="async" src={card.logo} alt="" />
                        </div>


                        {/* Placeholder for Person Image (Bottom/Right aligned) */}
                        {/* IMPORTANT: User will place the exact <img loading="lazy" decoding="async"> tags here */}
                        ...
                        <div className="relative z-20">
                            <p className={`text-xs sm:text-xs font-medium mb-1.5 ${card.batchColor}`}>
                                Batch 2024-2025
                            </p>
                            <div className={`font-display bg-gradient-to-r from-white/55 to-white/50 backdrop-blur-sm rounded-md p-2.5 sm:p-2 shadow-sm inline-block min-w-28 sm:min-w-40 border border-white/40`}>
                                <h3 className={`text-sm sm:text-lg font-medium text-gray-900 leading-tight`}>{card.name}</h3>
                                <p className="text-xs sm:text-sm text-[#0277bd] mt-0.5">{card.role}</p>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-12 lg:p-16 pb-0 md:pb-0 lg:pb-0">
                <div className="max-w-2xl w-full">
                    {/* Header Text */}
                    <h2 className={`font-display text-3xl md:text-4xl lg:text-5xl text-[#1F1F1F] leading-tight mb-6`}>
                        Placement Partners
                    </h2>
                    ...
                    <div className="flex flex-row justify-start items-center mb-16 relative gap-4 sm:gap-0">
                        ...
                        <div className="flex flex-col items-start w-1/2 sm:w-[45%]">
                            <span className={`font-display text-4xl sm:text-5xl lg:text-6xl font-light text-[#0277bd] leading-none mb-4`}>104 +</span>
                            <span className={`font-display bg-[#FFD812] text-foreground px-3 sm:px-4 py-1 sm:py-1.5 rounded-md text-xs sm:text-sm font-medium`}>Companies</span>
                            <p className="text-gray-500 text-xs sm:text-sm mt-4 leading-tight max-w-48">2023–24 Campus Placement Drive</p>
                        </div>

                        {/* Vertical Divider */}
                        <div className="block w-px h-24 sm:h-28 bg-gray-300 mx-2 sm:mx-8"></div>

                        {/* Stat 2 */}
                        <div className="flex flex-col items-start w-1/2 sm:w-[45%]">
                            <span className={`font-display text-4xl sm:text-5xl lg:text-6xl font-light text-[#0277bd] leading-none mb-4`}>206+</span>
                            <span className={`font-display bg-[#FFD812] text-foreground px-3 sm:px-4 py-1 sm:py-1.5 rounded-md text-xs sm:text-sm font-medium`}>Placements</span>
                            <p className="text-gray-500 text-xs sm:text-sm mt-4 leading-tight max-w-48">Offers Extended To Our Talented Graduates</p>
                        </div>
                    </div>
                    ...
                    <div className="relative flex flex-col gap-4 overflow-hidden">
                        ...
                        <Marquee reverse pauseOnHover className="[--duration:20s]">
                            {marqueeLogos.map((logo, idx) => (
                                <div key={idx} className="w-36 h-16 bg-background border border-gray-100 shadow-[0_4px_12px_rgba(0,0,0,0.03)] rounded-md flex items-center justify-center p-2">
