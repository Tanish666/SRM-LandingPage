...
export default function HeroSection5() {
    ...
    return (
        <section className="py-24 px-6 md:px-20 max-w-[1450px] mx-auto overflow-hidden bg-background">
            <div className="grid grid-cols-1 lg:grid-cols-[380px_minmax(0,1fr)] gap-10 lg:gap-14 max-w-7xl mx-auto items-start">
                ...
                <div className="bg-[#FFFDF1] rounded-lg overflow-hidden flex flex-col lg:h-[520px] h-[450px] pb-4">
                    <div className="bg-[#FFD700] rounded-lg py-2 text-center shadow-sm z-10 mx-1">
                        <h3 className={`font-display text-3xl text-foreground tracking-wide`}>Latest News</h3>
                    </div>
                    <div
                        ref={newsRef}
                    ...
                    >
                        {[...newsData, ...newsData].map((text, index) => (
                            <div key={index} className={`mb-5 pb-5 ${index !== (newsData.length * 2 - 1) ? 'border-b border-yellow-200/60' : ''}`}>
                                <p className="text-sm text-muted-foreground mb-2.5 leading-relaxed pr-2">
                                    {text}
                                </p>
                                <button className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors group">
                                    Read More
                                    <span className="bg-[#FFD700] rounded-full p-0.5 ml-1 group-hover:scale-110 transition-transform">
                                        <ArrowUpRight className="size-[10px] text-foreground" strokeWidth={2} />
                                    </span>
                                </button>
                            </div>
                        ))}
                ...
                <div className="flex flex-col">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 gap-4">
                        <h2 className={`font-display text-5xl lg:text-5xl text-foreground leading-[1.1]`}>
                            Insights From Our Learners
                        </h2>
                        <div className="flex gap-3 pb-1">
                            <button
                                onClick={handlePrev}
                                className="bg-[#FFD700] w-11 h-11 rounded-full flex items-center justify-center hover:bg-yellow-400 transition-colors shrink-0"
                            >
                                <IconArrowNarrowLeft className="text-white" size={30} strokeWidth={1.5} />
                            </button>
                            <button
                                onClick={handleNext}
                                className="bg-[#0073CF] w-11 h-11 rounded-full flex items-center justify-center hover:bg-blue-600 transition-colors shrink-0"
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[260px_minmax(0,1fr)] gap-6 lg:h-[400px] h-auto">
                        ...
                        <div className="hidden md:flex flex-col gap-3 h-full overflow-hidden">
                            <AnimatePresence mode="popLayout">
                                ...
                                <motion.div
                                    ...
                                    className="relative w-full h-24 rounded-3xl overflow-hidden group cursor-pointer"
                                    onClick={handlePrev}
                                >
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-4 flex flex-col justify-end">
                                        <h4 className={`font-display text-white font-extralight text-xl leading-tight`}>{getInsightAt(-1).name}</h4>
                                        <p className="text-white/80 text-xs mt-0.5 truncate">{getInsightAt(-1).program}</p>
                                    </div>
                                </motion.div>
                                ...
                                <motion.div
                                    ...
                                    exit={{ y: 20, opacity: 0 }}
                                    transition={{ duration: 0.5 }}
                                    className="relative w-full h-24 rounded-2xl overflow-hidden group cursor-pointer"
                                    onClick={handleNext}
                                >
                                    <Image src={getInsightAt(1).image} alt="Student" fill className="object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-4 flex flex-col justify-end">
                                        <h4 className={`font-display text-white  text-xl leading-tight`}>{getInsightAt(1).name}</h4>
                                        <p className="text-white/80 text-xs mt-0.5 truncate">{getInsightAt(1).program}</p>
                                    </div>
                                </motion.div>
                            </AnimatePresence>
                        ...
                        <motion.div
                            onClick={handleNext}
                            className="relative border border-slate-300/80 rounded-3xl p-5 sm:p-8 lg:p-10 h-full flex flex-col justify-start md:justify-center bg-background shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)] overflow-hidden group cursor-pointer"
                        >
                            ...
                            <motion.div
                                ...
                                className="absolute top-[55%] md:top-10 left-10 md:left-10 flex gap-1.5 md:gap-3 z-0"
                            >
                            ...
                            <div className="relative z-10 h-full flex flex-col">
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        {/* Mobile Image */}
                                        <div className="md:hidden relative w-full aspect-[3/1.9] rounded-3xl overflow-hidden mb-6 shadow-sm">
                                            <Image
                                        ...
                                        <p className="text-muted-foreground text-base lg:text-base leading-[1.8] mb-8 group-hover:text-muted-foreground transition-colors w-full sm:w-[95%]">
                                            {insights[activeIndex].quote}
                                        </p>
                                        <div className="flex flex-col mt-auto">
                                            <span className={`font-display text-[#3695EB] text-2xl font-medium mb-0.5`}>
