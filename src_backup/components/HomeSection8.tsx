...
const HomeSection8 = ({ coursesData }: { coursesData?: any[] }) => {
    ...
    return (
        <section className="relative w-full py-4 px-4 md:px-8 max-w-[1300px] mx-auto overflow-visible">
            <div className="relative bg-[#B9E3FF] rounded-3xl px-8 py-6 md:px-12 md:py-8 lg:px-8 lg:py-6 flex flex-col lg:flex-row items-center justify-between overflow-visible ">

                {/* Left Content */}
                <div className="relative z-10 w-full lg:w-1/2 mb-10 lg:mb-0 lg:pr-10">
                    <h2 className={`text-4xl md:text-4xl lg:text-5xl font-display text-[#1e2a33] leading-[1.1] mb-3 lg:whitespace-nowrap`}>
                ...
                {/* Right Content - Card */}
                <div className="relative w-full lg:w-96 bg-background rounded-2xl p-5 md:p-4 z-40 shadow-sm">
                    {/* Toggle Buttons */}
                    <div className="relative bg-[#FFEA00] rounded-full p-1.5 flex justify-between items-center mb-6 z-20">
                        <button
                            ...
                            className={`flex-1 font-medium py-3 rounded-full text-center text-sm transition-all ${activeTab === 'UG' ? 'bg-background text-[#111] shadow-sm' : 'text-[#222]'}`}
                        >
                            Under Graduate
                        </button>
                        <button
                            onClick={() => { setActiveTab('PG'); setSelectedCourse('SELECT COURSE'); }}
                            className={`flex-1 font-medium py-3 rounded-full text-center text-sm transition-all ${activeTab === 'PG' ? 'bg-background text-[#111] shadow-sm' : 'text-[#222]'}`}
                        >
                            Post Graduate
                        </button>
                    </div>
                    ...
                    <div className="relative z-50" ref={dropdownRef}>
                        ...
                        >
                            ...
                            <span className="text-foreground text-sm font-medium tracking-wider uppercase truncate max-w-52">
                                {selectedCourse}
                            </span>
                            <div className="flex-1 h-px bg-black"></div>
                            <svg
                        ...
                        {/* Dropdown Menu */}
                        {isOpen && (
                            <div className="absolute left-0 right-0 mt-1 bg-background border border-gray-100 rounded-xl shadow-2xl z-[100] max-h-48 overflow-y-auto custom-scrollbar">
                                {currentPrograms.map((program, index) => (
                                    <div
                                        key={`${program.id}-${index}`}
                                        onClick={() => handleSelect(program.name)}
                                        className="px-4 py-2.5 text-sm text-[#444] hover:bg-gray-50 hover:text-foreground cursor-pointer transition-colors border-b border-gray-50 last:border-none"
                                    >
                                        {program.name}
                                    </div>
                                ))}
                    ...
                    <div className="h-px w-full bg-[#E6E6E6] my-2" />
                    ...
                    <div className="relative flex justify-center my-3 z-20">
                        <button 
                            ...
                            className={`bg-[#FFD100] hover:bg-[#f0c500] text-foreground font-semibold py-2.5 px-10 rounded-full text-sm transition-colors shadow-sm active:scale-95 duration-200 ${selectedCourse === 'SELECT COURSE' ? 'opacity-50 cursor-not-allowed hover:bg-[#FFD100]' : ''}`}>
                            Explore Now
                        </button>
                    </div>
                </div>
