...
const DeptSection9 = ({ courseData }: Props) => {
    return (
        <section className="pt-20 px-6  max-w-[1440px] mx-auto w-full bg-background flex flex-col items-center">
            <SectionPill text="Achievements" className="mb-8" />
            {/* Title */}
            <h2
                className={`font-display text-5xl text-foreground text-center capitalize`}
                ...
                <div className="flex flex-col lg:flex-row justify-between gap-6 lg:gap-0 pb-4">
                    {cards.map((card, index) => (
                        <div key={index} className="w-full lg:w-96">
                            ...
                            {/* Title */}
                            <h3 className={`font-display text-2xl font-normal text-foreground capitalize mb-1`} style={{ lineHeight: '28px' }}>
                                {card.title}
                            </h3>

                            {/* Description */}
                            <p className={`${inter.className} text-base font-normal text-[#333333] mb-2`} style={{ lineHeight: '26px' }}>
                                {card.description}
                            </p>
