...
const ProgramsList = ({ coursesData }: { coursesData?: any[] }) => {
    ...
    return (
        <section className="flex flex-col items-center justify-center w-full px-8 md:px-20 py-16">
            ...
            <div className="w-full max-w-6xl mx-auto rounded-3xl border border-gray-200 overflow-hidden shadow-sm bg-[#EEEEEE]">
                {/* Header Bar */}
                <div className="bg-[#0071BC] p-4 lg:p-6 flex flex-col items-center justify-between gap-6 rounded-t-[21.04px] lg:flex-row">
                    ...
                    <div className="flex items-center bg-[#FFD812] rounded-full p-1 overflow-x-auto w-full lg:w-auto overflow-hidden shrink-0 no-scrollbar">
                        <button
                            ...
                            className={`flex-1 lg:flex-none px-4 lg:px-6 py-2 rounded-full font-medium whitespace-nowrap shadow-sm text-sm lg:text-base transition-colors ${activeTab === 'Under Graduate' ? 'bg-background text-gray-900' : 'text-gray-900 bg-transparent hover:bg-white/50'}`}
                        >
                            Under Graduate
                        </button>
                        <button
                            ...
                            className={`flex-1 lg:flex-none px-4 lg:px-6 py-2 rounded-full font-medium whitespace-nowrap shadow-sm text-sm lg:text-base transition-colors ${activeTab === 'Post Graduate' ? 'bg-background text-gray-900' : 'text-gray-900 bg-transparent hover:bg-white/50'}`}
                        >
                            Post Graduate
                        </button>
                    </div>
                    ...
                    <div className="flex-1 w-full max-w-2xl relative" ref={dropdownRef}>
                        <div className="flex items-center bg-background rounded-full overflow-hidden border border-gray-100 pl-4 lg:pl-6 py-1 pr-1 w-full relative">
                            <input
                                type="text"
                        ...
                        {showDropdown && suggestions.length > 0 && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-background rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50 max-h-80 overflow-y-auto">
                                {suggestions.map((suggestion, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs bg-gray-100 px-2 py-1 rounded-full text-gray-500">{suggestion.duration || "3 Year"}</span>
                                        </div>
                                    );
                                })}
                            </div>
                                ...
                                <div key={programId} className="px-6 md:px-10">
                                    <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-center py-6 border-b`} style={{ borderColor: '#c0c0c0' }}>
                                        <div className="mb-6 lg:mb-0 w-full lg:w-auto">
                                            <h3 className={`font-display text-lg lg:text-xl text-gray-900 mb-1`}>{program.name}</h3>
                                        ...
                                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                                            <button className="flex-1 lg:flex-none px-6 lg:px-8 py-2.5 bg-[#0070c0] text-white rounded-full font-medium text-xs lg:text-sm hover:bg-blue-700 transition-all whitespace-nowrap">
                                                Check Eligibility
                                            </button>
                                            <button className="flex-1 lg:flex-none px-6 lg:px-8 py-2.5 bg-[#FFD812] text-gray-900 rounded-full font-medium text-xs lg:text-sm hover:bg-yellow-400 transition-all shadow-sm whitespace-nowrap">
                                                Apply Now
                                            </button>
                                        </div>
                                    </div>
